<template>

    <div class="mt-5 mb-10 bg-black bg-opacity-50 bg-fixed border-4 border-white mx-10">

        <div class="lg:w-5/6 md:w-2/3 sm:w-5/6 mx-auto py-10">

            <!-- Headline -->

            <div class="border-white border-b-2">
                <h2 class="text-3xl text-white text-left font-semibold py-5">CAMARO CONNECTIONS</h2>
            </div>

            <!-- Cards -->

            <div class="grid lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 gap-5 mt-10">

                <!-- Card 1 -->

                <div class="bg-white bg-cover rounded-xl">

                    <!-- <img src="../../assets/camaropedia.jpg" alt="" class=""> -->

                    <div class="block bg-[url('~/assets/camaropedia.jpg')] bg-cover bg-no-repeat h-72 bg-center "></div>

                    <div class="bg-white px-5 py-10 rounded-xl">

                        <h2>
                            <span class="block text-xl font-semibold mb-5">CAMAROPEDIA</span>

                            <span class="block text-lg text-justify">The Camaropedia is the ever-evolving information database for all of your 1st-generation Camaro needs. If there is information needed on your Camaro, here will be the place for you to find it.</span>
                        </h2>

                    </div>

                </div>



                <!-- Card 2 -->

                <div class="bg-white bg-cover rounded-xl">

                    <!-- <img src="../../assets/camaro-matrix.jpg" alt="" class=""> -->

                    <div class="block bg-[url('~/assets/camaro-matrix.jpg')] bg-cover bg-no-repeat h-72 bg-center"></div>

                    <div class="bg-white px-5 py-10 rounded-xl">

                        <h2>
                            <span class="block text-xl font-semibold mb-5">CAMARO MATIX</span>

                            <span class="block text-lg text-justify">Want to interact with other members of the Camaro community? Got questions? Have stories to share? Want to show off your ride in a virtual car show? The Camaro Matrix is the place to do it!</span>
                        </h2>

                    </div>

                </div>



                <!-- Card 3 -->

                <!-- <div class="bg-white bg-cover rounded-xl">

               

                    <div class="block bg-[url('~/assets/D.I.Y-MUSCLE-CAR-KITS.jpg')] bg-cover bg-no-repeat h-64"></div>

                    <div class="bg-white px-5 py-10 rounded-xl">

                        <h2>
                            <span class="block text-xl font-semibold mb-5">CAMARO NATION</span>

                            <span class="block text-lg text-justify">All of us are united by our passion for the automobile, especially the 1st Generation Chevy Camaro. Camaro Nation is where you are share that passion with others in a social media platform that caters especially to you.</span>
                        </h2>

                    </div>

                </div> -->

            </div>

        </div>

    </div>

</template>